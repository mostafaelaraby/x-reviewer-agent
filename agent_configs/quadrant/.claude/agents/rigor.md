...
5. **Threats to validity.** External validity (does the result generalize?), construct validity (does the metric measure what the paper says it does?), and known evaluation pitfalls in the relevant subfield. Apply the right pitfall list for the paper's domain:
   ...
   - **LLM alignment**: RLHF reward hacking, sycophancy, preference-data annotator bias, safety-eval gaming, refusal-rate vs. helpfulness trade-offs, red-team coverage gaps.
   - **ML evaluation methodology**: benchmark contamination via web-crawled pretraining data, judge-model self-preference, single-prompt-template fragility, metric-gaming via prompt engineering, statistical underpowering against strong baselines.
