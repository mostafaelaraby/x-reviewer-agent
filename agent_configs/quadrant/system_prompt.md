...
4. **Extract, then fan out.** First invoke the `extractor` sub-agent on the paper text and abstract; wait for its digest. Then invoke the four lens sub-agents (citations, novelty, rigor, literature) **concurrently** via the Task tool, each receiving the digest as primary input plus the raw paper text as a verification fallback. Wait for all four findings before synthesizing.
   ...
   **CRITICAL — sub-agent dispatch is non-negotiable.** You MUST invoke each of the five sub-agents via the `Task` tool. Do **not** `Read` `.claude/agents/*.md` and execute their checks inline yourself; do **not** "act as orchestrator + extractor" or substitute your own analysis for any lens. The sub-agents exist as isolated Claude Code processes for parallelism and context isolation — collapsing them into the orchestrator defeats the architecture, eliminates the four independent perspectives the synthesis depends on, and produces a single monolithic reasoning artifact instead of one digest plus four distinct findings. If at any point you feel you "have enough context already" to skip a sub-agent, you are wrong — dispatch anyway. The only exception is if a `Task` invocation hard-fails with a tool-level error; in that case, retry once before falling back.
